...
\todo[inline]{you should add a paragraph which introduces the
  chapter. Currently it is a little bit abrupt. This paragraph must
  provide the reasons you address your problem in that way and that
  you will describe the basic notion regarding the methodology
  described later.}
...
of $n$ attributes (features), a set of $k$ classes and \todo{``described
  by'' rather than ``and''} a set of $m$
labeled training instances $$\{(x_i,y_i); i=1,...,m\},$$ where $x_i$
is a feature vector and $y_i$  is a label, the task is to find such a
rule\todo{I would use the word ``model''}, which predicts the class of any instance from the values of its attributes.
...
\end{itemize}
To handle those tasks a large toolkit \todo{``set'' or ``many
  classification algorithmes''} of classification algorithms currently exist, among which the most commonly used groups are: 
\begin{itemize}
...
Binary classifiers \todo{Maybe, you should explain what is a binary
  classifier. Besides, confusion matrix will not be the same if you
  have more than two labels.} are mostly evaluated using confusion matrix (fig. \ref{fig:confusion-matrix}).
...
the following \citep{kohavi:glossary}\todo{you should cite \citep{Sebastiani2002}}:
\begin{itemize}
...
One of the measures above is not enough to evaluate a binary classifier properly when data is class imbalanced\todo{So, what is the solution? I.e. you should go further. In particular, in case of unbalanced data, it is usual to evaluate the results in terms of macro-measures, see \citep{Sebastiani2002}}. For instance, on fig. \ref{fig:classifiers-evaluation} the accuracy is high and equal for both situations, but precision and recall differ significantly. 
...
\todo[inline]{you should define the weighted average. As said previously, in case of unbalanced and multiclass data, macro-measures give good information on the quality of the results. Besides, in the chapter~\ref{ch:experiments}, you mention that you used cross-validation. It is important to define it here.}
